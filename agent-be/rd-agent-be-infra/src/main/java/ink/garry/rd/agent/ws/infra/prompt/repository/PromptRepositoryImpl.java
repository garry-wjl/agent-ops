package ink.garry.rd.agent.ws.infra.prompt.repository;
 ...
 * Prompt 聚合仓储实现。
 * <p>
 * 参照 {@code ToolRepositoryImpl} / {@code SkillRepositoryImpl}：业务字段<b>仅注入
 * {@link PromptMapper}</b>，加上 {@link RedissonClient}（横切，用于写操作互斥锁，避免重试导致
 ...
 * （Gateway / Publisher）由 {@code PromptFactory} 装配。
 * <p>
 * <b>写互斥设计</b>：{@link #save} / {@link #deleteByNum} 共用同一把锁
 * （前缀 {@link LockKeyConstant#PROMPT_SAVE_LOCK_PREFIX}，按 num 维度）。
 * <p>
 * <b>删除语义</b>：全局 MyBatis-Plus logic-delete 配置生效，{@link #deleteByNum} 生成
 * {@code UPDATE ... SET deleted=1}；配合 {@code uq_prompt_ws_key(workspace_num, prompt_key,
 * deleted)} 唯一索引允许同 key Prompt 在删除后重建。
 */
@Slf4j
@Repository
public class PromptRepositoryImpl implements PromptRepository {
     ...
     * 按现有 infra 范式（{@code ToolRepositoryImpl#CODE_CONFLICT}）就地定义。
     */
     ...
     *
     * @param aggregate 待保存的 Prompt；必须已带业务编号 num；保存成功后回填自增 id
     * @throws BusinessException 抢锁失败（code {@value #CODE_CONFLICT}）或线程中断
     */
    @Override
    public void save(Prompt aggregate) {
        Assert.notNull(aggregate, "Prompt 聚合不能为 null");
        String num = aggregate.getNum();
        Assert.notBlank(num, "Prompt num 不能为空");

        runWithWriteLock(num, () -> {
            // 按业务编号查 DB 判定存在性（MyBatis-Plus 全局 logic-delete 自动追加 deleted=0）
            PromptEntity existing = promptMapper.selectOne(new LambdaQueryWrapper<PromptEntity>()
                    .eq(PromptEntity::getNum, num));
            PromptEntity entity = PromptEntity.fromDomain(aggregate);
            if (existing == null) {
     ...
     * @return 已转换的 Prompt；不存在返回 null
     */
    @Override
    public Prompt findByNum(String num) {
        if (num == null || num.isBlank()) {
            return null;
        }
        PromptEntity entity = promptMapper.selectOne(new LambdaQueryWrapper<PromptEntity>()
                .eq(PromptEntity::getNum, num));
        return PromptEntity.toDomain(entity);
    }
    ...
    public void deleteByNum(String num) {
        Assert.notBlank(num, "Prompt num 不能为空");
        runWithWriteLock(num, () -> promptMapper.delete(new LambdaQueryWrapper<PromptEntity>()
                .eq(PromptEntity::getNum, num)));
    }

    /**
     * 以 Prompt num 维度抢分布式锁后执行写操作；写操作样板代码统一收口在此。
     *
     * @param num    Prompt 业务编号（锁粒度）
     * @param action 临界区操作
     * @throws BusinessException 抢锁失败或线程中断
     */
    private void runWithWriteLock(String num, Runnable action) {
        RLock lock = redissonClient.getLock(LockKeyConstant.PROMPT_SAVE_LOCK_PREFIX + num);
        boolean acquired;
        try {
            acquired = lock.tryLock(WRITE_LOCK_WAIT_SECONDS, WRITE_LOCK_LEASE_SECONDS, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new BusinessException(CODE_CONFLICT, "Prompt 写操作被中断");
        }
        if (!acquired) {
            log.warn("prompt write lock busy num={}", num);
            throw new BusinessException(CODE_CONFLICT, "Prompt 正在保存中，请稍后重试");
        }
        try {
            action.run();
        } finally {
